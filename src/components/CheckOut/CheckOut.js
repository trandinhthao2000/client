...
class CheckOut extends Component {
  ...
  submitOrder = async () => {
    MySwal.fire({
      ...
      text: "Bạn muốn thanh toán ngay bây giờ",
      icon: "warning",
      ...
      if (result.value) {
        ...
        if (res.name === "" || res.address === "" || res.phone === "") {
          return toast.error("Vui lòng điền đầy đủ thông tin trước khi thanh toán");
        }
  ...
  toggleCheckout = async () => {
    ...
    if (res.name === "" || res.address === "" || res.phone === "") {
      return toast.error("Hoàn thành biểu mẫu trước khi thanh toán");
    }
    let addressProvince;
    if (res.provinces && res.provinces.length) {
      res.provinces.map((item) => {
              ...
              <div className="col-lg-12">
                <div className="error-wrapper text-center ptb-50 pt-xs-20">
                  ...
                  <div>
                    <p>
                      <i>
                        Thông tin chi tiết về đơn đặt hàng của bạn đã được gửi đến email. Vui lòng kiểm tra email của bạn để kiểm tra trạng thái của nó.
                      </i>
                    </p>
                  </div>
                </div>
